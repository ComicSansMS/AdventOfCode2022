...
std::list<int64_t>::iterator cyclicAdvance(std::list<int64_t>& l, std::list<int64_t>::iterator it, int64_t i)
{
    ...
    if (offset > 0) {
        // forward skips one more element because it needs to skip over the element to be moved
...
{
    std::vector<Tracker> const trackers = buildTrackers(l);
    decode(l, trackers);
}
...
int64_t getSolution(std::list<int64_t> const& l)
{
